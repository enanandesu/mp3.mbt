...
8 kHz 混合块有一个明确的参考例外：固定 minimp3 的混合频带表会越过 576 个
频谱样本的边界，本库从其同版本长、短频带表推导出 72 个长块样本加 504 个
短块样本的有界布局。FFmpeg 也尚未实现该切换点，因此这条分支不能声称与未修改
参考解码器 PCM 等价。修正来源、对照方式与证据范围见 `internal/layer3/README.md`；
常规语料及其余分支仍使用未修改的固定 C 参考验收。

可另运行 `python tools/verify_mixed_8000.py`，用只修正两个 8 kHz 操作的 mpg123
进行 16 例补充数值对照；首次运行下载固定哈希源码，需要 make 与 Bash。此独立
参考仍存在 MS+intensity 组合差异，故该组合不属于补充工具的接受范围；原有测试
不会因此放宽阈值。第三方源码、构建及 PCM 都只保留在忽略的 `target/` 中。
